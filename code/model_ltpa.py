...
from blocks import ConvBlock, LinearAttentionBlock, ProjectorBlock, Interpolate
...
class AttnVGG_before(nn.Module):
    def __init__(self, im_size, num_classes, attention=True, normalize_attn=True, init='xavierUniform', interpolate=False):
        super(AttnVGG_before, self).__init__()
        if interpolate:
            self.interpolate = Interpolate(size=(im_size, im_size), mode='nearest')
        else:
